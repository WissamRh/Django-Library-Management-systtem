...
def check_book_not_issued_to_user(borrower, book):
    qs = IssueLog.objects.filter(
        book=book, borrower=borrower, deposit_date=None)
    if qs:
        return False
    return True
...
class IssueLogExportAsCSV(views.APIView):
    ...
    def get(self, request):
        ...
        if type_of_data == 'issued':
            issue_log_qs = IssueLog.objects.filter(
                issued_date__range=[start, end])
        elif type_of_data == 'deposited':
            issue_log_qs = IssueLog.objects.filter(
                deposit_date__range=[start, end])
        elif type_of_data == 'delayed':
            issue_log_qs = IssueLog.objects.filter(
                due_date__range=[start, end], penalty__gt=0)
        else:
            issue_log_qs = IssueLog.objects.filter(
                issued_date__range=[start, end])
