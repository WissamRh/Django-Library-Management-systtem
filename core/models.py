...
from .utlis import get_due_date
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    # indicating that this field is being used as the unique username indicator
...
class Book(models.Model):
    ...
    class Meta:  # meta is used for humanreadable language
        verbose_name = _("Book")
...
class IssueRequest(models.Model):
    ...
    requester = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, blank=False)
    book = models.ForeignKey(Book, on_delete=models.DO_NOTHING, blank=False)
    request_status = models.CharField(
        max_length=2, choices=RequestStatus.choices, default=RequestStatus.Requested,)
...
def send_book_issue_request_notification_email(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        message = f'Book : {book} has been requested to issue by {
            user} on {request_date}. \n Thanks!'

        send_mail(
            subject,
            message,
            email,
            ['wissamrh02@hotmail.com'],
            fail_silently=False
        )
...
def send_book_issue_notification_email(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        message = f'Book : {book} has been issued on {
            issued_date} due on {due_date}. \n Thanks!'
